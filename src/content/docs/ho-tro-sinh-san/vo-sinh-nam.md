...
```mermaid
flowchart TD
    Start([Tinh dịch đồ thực hiện ≥ 2 lần, cách 3 - 4 tuần]) --> Assess[Phân loại mức độ]
    Assess -->|Mức độ nặng| Heavy[Trữ đông tinh trùng & tư vấn IVF]
    Assess -->|Tất cả các mức độ| Screen[Thăm khám tầm soát nguyên nhân]
    Screen --> Hormone[Định lượng nội tiết sinh sản]
    Hormone -->|FSH > 12 IU/L và/hoặc LH > 12 IU/L| Hyper[Hypergonadotropin]
    Hyper --> Heavy
    Hormone -->|FSH < 12 IU/L và LH < 12 IU/L| Hypo[Điều trị nội khoa]
    Hypo --> Varicocele{Giãn tĩnh mạch thừng tinh?}
    Varicocele -->|Đủ chỉ định| Surgery[Phẫu thuật thắt tĩnh mạch]
    Varicocele -->|Không chỉ định| FollowUp[Theo dõi tinh dịch đồ mỗi tháng trong 6 - 12 tháng]
    Surgery --> FollowUp
    FollowUp --> Improvement{Đánh giá cải thiện tinh dịch đồ}
    Improvement -->|Có cải thiện| IUI[Bơm tinh trùng vào buồng tử cung - IUI]
    Improvement -->|Không cải thiện / Yếu nặng| IVF[Thụ tinh ống nghiệm - IVF]
```

_Hình "Sơ đồ xử trí thiểu tinh (tinh trùng yếu, ít, dị dạng) bằng Mermaid"_.

<details>
<summary>Xem lược đồ gốc từ phác đồ Bệnh viện Hùng Vương</summary>

![Lưu đồ xử trí tinh trùng yếu, ít, dị dạng](./_images/vo-sinh-nam/hung-vuong-luu-do-xu-tri-thieu-tinh.png)
_Hình "Lưu đồ xử trí tinh trùng yếu, ít, dị dạng"_.
...
```mermaid
flowchart TD
    Start([Tinh dịch đồ ≥ 2 lần, cách 3 - 4 tuần]) --> Az[Xác nhận vô tinh]
    Az --> Classify{Phân loại vô tinh?}
    Classify -->|Vô tinh do tắc| Obst[Tìm vị trí tắc nghẽn & lên kế hoạch phẫu thuật phục hồi]
    Classify -->|Vô tinh không do tắc| NonObst[Tầm soát di truyền: Karyotype, vi mất đoạn NST Y...]
    NonObst --> Genetic{Phát hiện bất thường?}
    Genetic -->|Có bất thường| GeneticCounsel[Tư vấn di truyền & xin tinh trùng hiến hoặc xin con nuôi]
    Genetic -->|Không bất thường| Hormone2[Định lượng nội tiết sinh sản]
    Hormone2 -->|Hypogonadotropin| Hypo2[Bổ sung Gonadotropin ngoại sinh]
    Hormone2 -->|Gonadotropin bình thường hoặc tăng| Normal[Điều trị theo chỉ số Testosterone và tỷ lệ T/E]

    Hypo2 --> Target{Đạt mục tiêu: T > 8 ng/mL, T/E > 10 kéo dài ≥ 3 tháng, có tinh trùng?}
    Normal --> Target

    Target -->|Có tinh trùng| Maintain[Duy trì điều trị 6 - 12 tháng tiếp theo]
    Target -->|Không tinh trùng nhưng đạt mục tiêu nội tiết| MicroTESE[Tiến hành phẫu thuật MicroTESE]
    Target -->|Thất bại hoàn toàn điều trị nội khoa| Unsuccessful[Ngưng điều trị nội khoa & tư vấn xin tinh trùng/con nuôi]

    Maintain --> Choose[Chỉ định IUI hoặc IVF tùy mức độ tinh trùng]
    MicroTESE --> Choose
```

_Hình "Sơ đồ xử trí vô tinh bằng Mermaid"_.

...
- **Nhiễm khuẩn tiết niệu:** Phát hiện vi khuẩn GBS trong nước tiểu (bất kể số lượng khuẩn lạc, ngay cả **< 10⁵ CFU/mL**) ở bất kỳ thời điểm nào của thai kỳ hiện tại.
...
```mermaid
flowchart TD
    A[Thai phụ có chỉ định IAP dị ứng Penicillin] --> B{Nguy cơ phản vệ cao?}

    B -- "Không (Nguy cơ thấp:<br>ban da nhẹ, không ngứa/sốc)" --> C["cefazolin 2 g IV liều đầu,<br>sau đó 1 g IV mỗi 8 giờ"]

    B -- "Có (Nguy cơ cao:<br>sốc phản vệ, mày đay, phù Quinke)" --> D{Kết quả thử nghiệm độ nhạy<br>Clindamycin?}

    D -- "Nhạy cảm với Clindamycin" --> E["clindamycin 900 mg IV<br>mỗi 8 giờ"]
    D -- "Kháng Clindamycin / Không rõ" --> F["vancomycin 20 mg/kg IV<br>mỗi 8 giờ (tối đa 2 g/liều)"]
```
